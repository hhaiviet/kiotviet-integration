...
if (-not (Test-Path ".\deploy\fully_automated_deploy.sh")) {
    Write-Error2 "Please run this script from the kiotviet-integration project root directory"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Warning2 "SSH connection test failed - continuing anyway"
        Write-Info "You may need to setup SSH keys or check network connectivity"
    }
} catch {
    Write-Warning2 "SSH test failed: $($_.Exception.Message)"
}

# Check for required tools
Write-Log "--- Checking deployment tools..." $Blue
...
if (-not $bashFound) {
    Write-Error2 "Neither WSL nor Git Bash found"
...
try {
    python pre_deploy_check.py 2>$null | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-Warning2 "Pre-deployment check found issues, but continuing..."
    }
} catch {
    Write-Warning2 "Could not run pre-deployment check: $($_.Exception.Message)"
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Error2 "Deployment failed with exit code: $LASTEXITCODE"
        Write-Info "Check the output above for error details"
        exit 1
    }
} catch {
    Write-Error2 "Deployment execution failed: $($_.Exception.Message)"
    exit 1
}
...
try {
    python remote_debug.py status 2>$null
    Write-Success "Post-deployment check completed"
} catch {
    Write-Warning2 "Could not run post-deployment check: $($_.Exception.Message)"
...
Write-Host "=========================================================" -ForegroundColor $Purple
Write-Host ""
Write-Host "[INFO] Target: $PI_USER@$PI_IP" -ForegroundColor $Blue
Write-Host "[INFO] KiotViet User: $Username" -ForegroundColor $Blue
Write-Host "[INFO] Project Path: /home/$PI_USER/$PROJECT_NAME" -ForegroundColor $Blue
Write-Host ""
Write-Host "[TOOLS] Remote Management Commands:" -ForegroundColor $Yellow
...
Write-Host ""
Write-Host "[AUTO] The system will automatically:" -ForegroundColor $Green
...
Write-Host ""
Write-Host "[HELP] If you need help:" -ForegroundColor $Yellow
...
Write-Host ""
Write-Host "[DONE] Your KiotViet integration is now running 24/7!" -ForegroundColor $Green