...
# Modern HTML Dashboard with improved design
DASHBOARD_HTML = """
<!DOCTYPE html>
<html><head>
...
<script>
const MAX_DURATION = 15; // seconds - for progress bar scaling

function getProgressPercent(duration) {
  return Math.min((duration / MAX_DURATION) * 100, 100);
}
...
function refresh() {
  fetch('/api/data')
    ...
    .then(d => {
      ...
      if (d.latest) {
        const lt = d.latest;
        document.getElementById('lt-time').textContent = lt.timestamp || '-';
        document.getElementById('lt-products').textContent = lt.product_count || 0;
        document.getElementById('lt-invoices').textContent = lt.invoice_lines || 0;
        document.getElementById('lt-duration').textContent = lt.total_duration ? lt.total_duration.toFixed(1) : '0.0';
        
        // Update progress bars
        const invDur = lt.invoice_lines || 0; // placeholder - use actual invoice duration if available
        const prodDur = lt.product_count ? 6 : 0; // approximate product duration
        const invPercent = getProgressPercent(2); // ~2s invoice
        const prodPercent = getProgressPercent(6); // ~6s products
        
        document.getElementById('inv-progress').style.width = invPercent + '%';
        document.getElementById('prod-progress').style.width = prodPercent + '%';
      }
      ...
      if (d.last_10 && d.last_10.length > 0) {
        d.last_10.reverse().forEach((r, i) => {
          const st = r.status === 'success' ? '✅ Success' : '❌ Failed';
          const sc = r.status === 'success' ? 'status-success' : 'status-failed';
          const dur = r.total_duration ? r.total_duration.toFixed(1) : '0.0';
          h += `<tr>
            <td>${d.last_10.length - i}</td>
            <td><span class="status-badge ${sc}">${st}</span></td>
            <td class="time-cell">${r.timestamp}</td>
            <td>${r.product_count}</td>
            <td>${r.invoice_lines}</td>
            <td class="duration-cell">${dur}s</td>
          </tr>`;
        });
      }
...
<div class="stats-grid">
  <div class="stat-card">
    ...
    <div class="stat-value"><span id="today-count">0</span></div>
  </div>
  <div class="stat-card">
    <div class="stat-label">Successful</div>
    <div class="stat-value"><span id="today-success">0</span><span class="stat-unit">✓</span></div>
  </div>
  <div class="stat-card">
...
def api_dashboard():
    ...
    return jsonify({
        "latest": {
            ...
            "invoice_lines": latest.invoice_lines if latest else 0,
            "total_duration": latest.total_duration if latest else 0,
        } if latest else {"timestamp": "-", "status": "unknown", "product_count": 0, "invoice_lines": 0, "total_duration": 0},
        "today": {
            "count": len(today_runs),
            "successful": len([r for r in today_runs if r.is_success]),
        },
